...
void top::rob_mode(unsigned int nadd, unsigned int nmul, unsigned int nload, map<string, int> instruct_time, vector<string> instruct_queue, nana::listbox &table, nana::grid &mem_gui, nana::listbox &regs, nana::listbox &instr_gui, nana::label &ccount, nana::listbox &rob_gui, unsigned int pred_tp)
{
    ...
    rob = unique_ptr<reorder_buffer>(new reorder_buffer("rob", rob_size, 2, rob_gui, instr_gui.at(0), pred_tp));
...
void top::show_metricas()
{
    ...
    int inst_total = (rob->get_n_commited_inst() + fila_r->get_inst_commit_count());
    double cpi = float(ciclos) / inst_total;
    double ipc = inst_total / float(ciclos);
    double cpu_time = inst_total * cpi * 1 / 1000000000;
    // double cpu_time = float(ciclos) * 1 / 1000000000;
    double mips = (cpu_time / cpi) / 1000000;

    cout << "\n\n--------------------MÉTRICAS--------------------\n\n";
    cout << "Instruções Comitadas: " << inst_total << "\n";

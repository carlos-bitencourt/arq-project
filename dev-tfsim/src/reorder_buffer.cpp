...
reorder_buffer::reorder_buffer(sc_module_name name, unsigned int sz, unsigned int pred_size, nana::listbox &gui, nana::listbox::cat_proxy instr_gui, unsigned int pred_tp) : sc_module(name),
                                                                                                                                                                             tam(sz),
                                                                                                                                                                             preditor(pred_size, pred_tp),
                                                                                                                                                                             gui_table(gui),
...
void reorder_buffer::leitura_issue()
{
    ...
    while (true)
    {
        ...
        if (ptrs[pos]->busy)
        {
            cout << "ROB esta totalmente ocupado" << endl
                 << flush;
            wait(free_rob_event);
        }
        in_issue->read(p);
        ...
        cout << "Inserindo instrucao " << p << " no ROB " << pos + 1 << "|" << sc_time_stamp() << endl
             << flush;
        ptrs[pos]->busy = true;
...
bool reorder_buffer::branch(int optype, int rs, int rt)
{
    ...
    case 1:
        ...
        return false;
    default:
        cerr << "Erro inesperado, abortando..." << endl
             << flush;
        sc_stop();
        nana::API::exit();
        exit(1);
